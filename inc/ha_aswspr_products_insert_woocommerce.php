...
function products_insert_woocommerce_callback()
{
    ...
    foreach ($products as $product) {
        ...
        update_post_meta($product_id, '_regular_price', $product_data['StandardPrice']);
        update_post_meta($product_id, '_price', $product_data['StandardPrice']);
        ...
        // Display out of stock message if stock is exactly 0
        if ($stock == 0) {
            update_post_meta($product_id, '_stock_status', 'outofstock');
        } else {
            // For any other stock values (positive or negative), set the status to 'instock'
            update_post_meta($product_id, '_stock_status', 'instock');
        }
        // Manage stock
        update_post_meta($product_id, '_manage_stock', 'yes');
